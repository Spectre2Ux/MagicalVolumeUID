...
int main() {
	if (!GetPrivileges()) {
		MessageBox(NULL, "Please Open Program As Administrator!", "Access Information", MB_ICONWARNING);
		return FALSE;
	}
...
BOOL GetPrivileges() {
	...
	if (!OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &TokenHandle)) {
		MessageBox(NULL, "Unexpected error; unable to retrieve process information.", "Process Information Unexpected.", MB_ICONERROR);
		WriteLog("Unexpected error; unable to retrieve process information.");
		return FALSE;

	}

	TOKEN_ELEVATION CheckElevated = { 0 };
	DWORD BytesReturned = { 0 };
		if (GetTokenInformation(TokenHandle, TokenElevation, &CheckElevated, sizeof(CheckElevated), &BytesReturned)) {
			ElevatedStatus = CheckElevated.TokenIsElevated;
	}
	WriteLog("User rights successfully verified!");
	return CheckElevated.TokenIsElevated;
}